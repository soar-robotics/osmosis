	...
	for name, test := range tests {
		s.Run(name, func() {
			...
			if test.expectErr {
				...
				newLowerTick := math.PriceToTick(test.newLowerPrice, pool.GetPrecisionFactorAtPriceOne())
				newUpperTick := math.PriceToTick(test.newUpperPrice, pool.GetPrecisionFactorAtPriceOne())

				// TODO: Dont hardcode
				lowerSqrtPrice, err := math.TickToSqrtPrice(newLowerTick, sdk.NewInt(-4))
				s.Require().NoError(err)
				upperSqrtPrice, err := math.TickToSqrtPrice(newUpperTick, sdk.NewInt(-4))
				s.Require().NoError(err)
